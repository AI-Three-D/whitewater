...
(async function main() {
  ...
  // obstacles: solid and depth-writing like the props, but alpha-blended so a despawning log can
  // sink and fade at once, and so one can fade in at the edge of the draw range
  ...
  // ---------- floating obstacles ----------
  // Instance storage is a fixed pool sized by the population cap, allocated once at load: the
  // live list is re-bucketed by mesh every frame and written contiguously, so no buffer is ever
  // resized and nothing is drawn for a slot that isn't in use.
  const OBST_SLOTS = OBSTACLES.maxAlive + 6;
  const obstMeshes = {}; for (const [k, mb] of Object.entries(buildObstacleMeshes())) obstMeshes[k] = gpuMesh(mb);
  const obstBodyNames = Object.keys(obstMeshes).filter(n => n !== 'stub');
  const obstInstBufs = {}, obstInstData = {}, obstDrawCount = {};
  for (const name of obstBodyNames) {
    obstInstBufs[name] = mkBuf(OBST_SLOTS * 80, GPUBufferUsage.VERTEX | GPUBufferUsage.COPY_DST);
    obstInstData[name] = new Float32Array(OBST_SLOTS * 20);
    obstDrawCount[name] = 0;
  }
  const OBST_STUB_MAX = OBST_SLOTS * OBSTACLES.stubsMax;
  const obstStubBuf = mkBuf(OBST_STUB_MAX * 80, GPUBufferUsage.VERTEX | GPUBufferUsage.COPY_DST);
  const obstStubData = new Float32Array(OBST_STUB_MAX * 20);
  let obstStubCount = 0;
    ...
    step(dt) {
      ...
      if (river.obstNear) for (const ob of river.obstNear) {
        ...
        for (const lp of OBST_HULL_PTS) {
          ...
          const f = [nx * fn, ob.bumpLift * fn, nz * fn];
          addForceAt(pw, f);
  ...
  function resetObstacles() {
    river.obstacles = [];
    river.obstNear = [];
    river.obstHorizon = 20;            // just past the put-in; the first sweep fills from here
    river.obstPend = {};
    river.obstRng = mulberry32(river.seed + 501);
  }
  // Sweeps a spawn horizon OBSTACLES.spawnAhead metres downstream of the boat and seeds each
  // newly uncovered metre of river at the configured per-100 m density. Nothing is ever created
  // inside the draw range, so an obstacle is always already floating by the time it's visible.
  // The first call of a run fills the entire initial stretch in one go (all of it off-screen).
  function spawnObstacles() {
    const cfg = river.R.obstacles;
    if (!cfg || !OBSTACLES.enabled) return;
    const maxAlive = cfg.max ?? OBSTACLES.maxAlive;
    const h0 = river.obstHorizon, h1 = Math.min(kayak.p[2] + OBSTACLES.spawnAhead, river.finishZ - 10);
    if (h1 <= h0) return;
    river.obstHorizon = h1;
    const rng = river.obstRng, swept = h1 - h0;
    for (const [kindName, kindCfg] of Object.entries(cfg)) {
      const kind = OBSTACLES.kinds[kindName];
      if (!kind) continue;                         // skips non-kind keys such as `max`
      for (const clsName of Object.keys(OBSTACLES.classes)) {
        ...
        const key = kindName + '.' + clsName;
        river.obstPend[key] = (river.obstPend[key] ?? 0) + swept * (spec.per100m ?? 0) / 100;
        while (river.obstPend[key] >= 1) {
          river.obstPend[key] -= 1;
          // at the cap, stop and don't bank an unbounded backlog that would dump all at once
          if (river.obstacles.length >= maxAlive) { river.obstPend[key] = Math.min(river.obstPend[key], 1); break; }
          addObstacle(kind, kindName, clsName, spec, h0 + rng() * swept, rng);
        }
      }
    }
  }
  function addObstacle(kind, kindName, clsName, spec, z, rng) {
    const base = kind[clsName], cls = OBSTACLES.classes[clsName];
    if (!base) return;
    const pond = river.R.pond;
    if (pond && z > pond.z - pond.len / 2 - 5 && z < pond.z + pond.len / 2 + 5) return;   // keep the pond clear
    ...
    for (let tries = 0; tries < 8 && !ok; tries++) {
      ...
      // don't drop one on top of another: they'd shove each other across the river on frame one
      ok = river.obstacles.every(o => Math.hypot(o.x - x, o.z - z) > (o.len + len) * 0.5 + 1);
    }
    if (!ok) return;
    const stubs = [];
    if (kind.stubs) for (let s = 0, n = Math.floor(rng() * (OBSTACLES.stubsMax + 1)); s < n; s++) stubs.push({
      t: (rng() - 0.5) * 0.85,                 // along the trunk, as a fraction of its length
      phi: rng() * 6.2832,                     // around the trunk
      pitch: (rng() - 0.5) * 1.5,              // swept toward one end or the other
      len: OBSTACLES.stubLen[0] + rng() * (OBSTACLES.stubLen[1] - OBSTACLES.stubLen[0]),
      rad: 0.028 + rng() * 0.035,
    });
    const g = 0.85 + 0.3 * rng(), t = kind.tint;
    const yScale = rad * radY;
    river.obstacles.push({
      kind: kindName, cls: clsName, mesh: base.meshes[Math.floor(rng() * base.meshes.length)],
      len, rad, radY, stubs,
      mass: Math.max(8, kind.density * kind.volFactor * rad * rad * radY * len),
      draft: Math.max(0.06, yScale * kind.draftFrac),
      freeboard: yScale * kind.freeboardFrac,
      samples: cls.samples, hitK: cls.hitK, bumpLift: cls.bumpLift,
      x, z, y: null, yaw: rng() * 6.2832, roll: kind.roll ? rng() * 6.2832 : 0,
      vx: 0, vz: 0, w: (rng() - 0.5) * 0.2, bobPh: rng() * 6.2832,
      fx: 0, fz: 0, tq: 0, grounded: false, sink: null,
      tint: [t[0] * g, t[1] * g, t[2] * g, 1],
    });
  }
  // one body, one substep (see the OBSTACLES comment for what the drag anisotropy is doing)
  ...
  // both ways per pair, which is what lets several logs stack behind a jammed one instead of
  ...
  function updateObstacles(dtReal) {
    ...
    if (!list || !list.length) { river.obstNear = []; return; }
    const zk = kayak.p[2], steps = OBSTACLES.substeps, dt = dtReal / steps;
    // exit: anything well behind the boat (or past the take-out) sinks away while fading out,
    // then leaves the list — nothing ever simply blinks off
    for (let i = list.length - 1; i >= 0; i--) {
      const ob = list[i];
      if (ob.sink == null && (ob.z < zk - OBSTACLES.despawnBehind || ob.z > river.finishZ + 20)) ob.sink = 0;
      else if (ob.sink != null && (ob.sink += dtReal / OBSTACLES.sinkTime) >= 1) list.splice(i, 1);
    }
    for (let s = 0; s < steps; s++) {
      for (const ob of list) stepObstacle(ob, dt);
      for (let a = 0; a < list.length; a++) for (let b = a + 1; b < list.length; b++) {
        const A = list[a], B = list[b], reach = (A.len + B.len) / 2 + A.rad + B.rad;
        if (Math.abs(A.z - B.z) > reach || Math.abs(A.x - B.x) > reach) continue;
        contactSegs(A, B, dt); contactSegs(B, A, dt);
      }
    }
    for (const ob of list) { ob.fx = 0; ob.fz = 0; ob.tq = 0; }
    // broad phase for the hull contact test, consumed by kayak.step next frame
    river.obstNear = list.filter(ob => ob.sink == null && ob.hitK > 0 &&
      Math.hypot(ob.x - kayak.p[0], ob.z - kayak.p[2]) < ob.len / 2 + ob.rad + 4);
  }
  function writeObstacleInstances(dtReal) {
    for (const name of obstBodyNames) obstDrawCount[name] = 0;
    obstStubCount = 0;
    const list = river.obstacles;
    if (!list || !list.length) return;
    const zk = kayak.p[2];
    const kSm = 1 - Math.exp(-dtReal * OBSTACLES.ySmooth);
    for (const ob of list) {
      // The drawn height is low-pass filtered, never taken raw from the sampled surface:
      // waterAt() switches between the live simulated band around the boat and the channel's
      // normal-depth estimate outside it, and the two don't agree to the centimetre, so reading
      // it directly made a log hop vertically each time the moving band swept over it. The
      // freeboard term then holds most of the hull above the wavy surface, so the water pass
      // (drawn later, and depth-writing) can't swallow it and flicker it away.
      const target = waterAt(ob.x, ob.z).eta + ob.freeboard;
      ob.y = ob.y == null ? target : ob.y + (target - ob.y) * kSm;
      // past the end of the drawn terrain there is no water under them, so they aren't drawn at
      // all — with a short alpha ramp inside the fog so the boundary itself is invisible
      const ahead = ob.z - zk;
      const edge = ahead > 0 ? RENDER.viewAhead - ahead : RENDER.viewBehind + ahead;
      if (edge <= 0) continue;
      let alpha = clamp(edge / OBSTACLES.drawFade, 0, 1);
      let y = ob.y + (ob.grounded ? 0 : OBSTACLES.bob * Math.sin(simTime * OBSTACLES.bobSpeed + ob.bobPh));
      if (ob.sink != null) {
        const t = clamp(ob.sink, 0, 1);
        y -= OBSTACLES.sinkDepth * ob.rad * t * t;     // slides under, accelerating
        alpha *= 1 - t;                                 // …while fading, at the same time
      }
      if (alpha <= 0.004) continue;
      const q = qMul(qAxisAngle([0, 1, 0], ob.yaw), qAxisAngle([0, 0, 1], ob.roll));
      const tint = [ob.tint[0], ob.tint[1], ob.tint[2], alpha];
      const n = obstDrawCount[ob.mesh];
      if (n < OBST_SLOTS) {
        const d = obstInstData[ob.mesh];
        d.set(mat4Compose([ob.x, y, ob.z], q, [ob.rad, ob.rad * ob.radY, ob.len]), n * 20);
        d.set(tint, n * 20 + 16);
        obstDrawCount[ob.mesh] = n + 1;
      }
      // Branch stubs: placed in world space here rather than baked into the log mesh, because the
      // log's instance scale is ~20:1 along its axis and would stretch any off-axis geometry into
      // a flat fin. Visual only — nothing in the collision path knows they exist.
      for (const st of ob.stubs) {
        if (obstStubCount >= OBST_STUB_MAX) break;
        const c = Math.cos(st.phi), s2 = Math.sin(st.phi), rL = ob.rad * 0.7;
        const pos = v3.add([ob.x, y, ob.z], qRotate(q, [c * rL, s2 * rL, st.t * ob.len]));
        // +Z (the stub mesh's own axis) → radially outward at angle phi, swept by `pitch`
        const qs = qMul(q, qMul(qAxisAngle([0, 0, 1], st.phi), qAxisAngle([0, 1, 0], Math.PI / 2 - st.pitch)));
        obstStubData.set(mat4Compose(pos, qs, [st.rad, st.rad, st.len]), obstStubCount * 20);
        obstStubData.set(tint, obstStubCount * 20 + 16);
        obstStubCount++;
      }
    }
    for (const name of obstBodyNames) if (obstDrawCount[name])
      device.queue.writeBuffer(obstInstBufs[name], 0, obstInstData[name], 0, obstDrawCount[name] * 20);
    if (obstStubCount) device.queue.writeBuffer(obstStubBuf, 0, obstStubData, 0, obstStubCount * 20);
  }
  ...
  function frame(now) {
    ...
    writeObstacleInstances(dtReal);   // also while 'over', so the wreck scene keeps its logs
    updateSparks(dtReal);
    // particle emitters
    ...
    // floating obstacles: own pipeline (alpha-blended, still depth-writing), drawn before the
    // water so the submerged part of each one is correctly covered by the surface
    pass.setPipeline(obstPipe);
    for (const name of obstBodyNames) {
      const n = obstDrawCount[name];
      if (!n) continue;
      pass.setVertexBuffer(0, obstMeshes[name].vbuf); pass.setVertexBuffer(1, obstInstBufs[name]);
      pass.draw(obstMeshes[name].count, n);
    }
    if (obstStubCount) {
      pass.setVertexBuffer(0, obstMeshes.stub.vbuf); pass.setVertexBuffer(1, obstStubBuf);
      pass.draw(obstMeshes.stub.count, obstStubCount);
    }
